...
It isn't perfectly tidy even then, and I'd rather say so than round it off. The commit the board port pins is 15.05.**1**. The feed it builds against, and the boards sitting on my desk, both say 15.05-**rc2**. So the source you can clone today isn't quite the source my boards were built from. I don't know why and I didn't chase it - but that's the sort of loose end an abandoned project leaves behind, where the repo kept moving for a while after the hardware stopped.
