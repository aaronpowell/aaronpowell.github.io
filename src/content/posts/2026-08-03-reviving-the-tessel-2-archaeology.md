...
So this post is the archaeology. What's actually on these boards, how the pieces fit together, and where the seams are - because nearly every problem I hit over the following weeks turned out to be grounded here.
...
The MediaTek MT7620n is the one I mentioned in [the last post]({{< ref "/posts/2026-07-31-reviving-the-tessel-2-the-boards-in-the-drawer.md" >}}). It's a MIPS32 SoC (System on a Chip), the same class of chip you'd find inside a cheap home wifi router, and it does the router-shaped things - wifi, Ethernet, the two USB host ports - plus it runs Linux and it runs your JavaScript.
...
The second reason is the one I came to appreciate a _lot_ over the following weeks: the SAMD21 can reprogram the SoC's SPI flash over USB. That means a bad Linux image is always recoverable. You can push a completely broken OS onto the board, watch it fail to boot, and then just... put a different one on (and trust me, this will become **very** important). From the host. Over the same USB cable.
...
Now that we know there are two chips that talk to each other, how do they do it? The short answer is SPI, which is accurate and also not very useful. Here's what actually happens.
...
- **MediaTek MT7620n**, MIPS 24KEc core, roughly 580MHz, and no floating-point unit - which turns out to matter more than anything else on this list.
- **64 MB of RAM, 32 MB of SPI-NOR flash.**
- **OpenWrt Chaos Calmer 15.05** - though its package feed points at `15.05-rc2`, which I'll come back to. Either way it's 2015, and these boards never moved off it.
- **Linux kernel 3.18.**
...
The whole OS is a squashfs image of about 4.3 MB, which is a genuinely impressive bit of engineering and also the source of a lot of constraints later on. It's worth sitting with those two numbers together: 4.3 MB of operating system in 32 MB of flash. There's room on that chip for several entire copies of the OS, which is the practical reason the recovery story works - a spare image and a bootloader will always fit alongside whatever you've just broken.

There's a detail in the shipped image I keep coming back to. Its own `/etc/opkg.conf` points opkg at `http://downloads.openwrt.org/chaos_calmer/15.05-rc2/ramips/mt7620/packages`, and [the build config](https://github.com/aaronpowell/openwrt-tessel/blob/d81c023d18e85bc3ea36b379c7dbb46d12c256ae/config.mk#L77) that produced it says the same thing. Plaintext HTTP, and a feed for a release candidate that stopped existing years ago. So the boards were shipped pointing at a package repository that was always going to disappear. The plaintext transport matters less than it looks, mind you - opkg was set to verify package signatures, and both the OpenWrt release key and Tessel's own build key are still sitting in `/etc/opkg/keys`. It's the disappearing that's the problem, not the HTTP.

None of this was negligence, to be clear. It was 2015, the project was moving fast, and pinning your package feed to the release candidate you were building against is an entirely normal thing to do when you fully intend to move it along next month. The problem isn't the decision, it's that nobody ever got to make the next one.
...
1. **An operating system with an actual patch stream.** Not a 2015 release nobody has touched since. Something still being maintained.
...
The tempting story is "the documentation was lost and AI recovered it", but that's not what happened. As I mentioned, the information was all there. Seven repos, some still working docs sites (and Wayback machine for getting desperate) - the source for everything was available.

What AI actually did was _reconcile_ it. With so many places that contained partial, overlapping, or differently-aged descriptions of the same device, some of them stale, some of them contradicting each other - all checked against what the live board actually reported when you asked it. That's the job. It isn't retrieval, it's resolution, and it's the kind of tedious cross-referencing work that I would simply never have done on a Tuesday night.
...
It isn't perfectly tidy even then, and I'd rather say so than round it off. The commit the board port pins is 15.05.**1**. The image that actually shipped says, in its own `/etc/openwrt_version`, plain **15.05** - and that same image's `opkg.conf` points the package feed at 15.05-**rc2**. Three answers, one image. So the source you can clone today isn't quite the source my boards were built from. That's the sort of loose end an abandoned project leaves behind, where the repo kept moving for a while after the hardware stopped.

Both links above are pinned to a specific commit, because the wrong one has since been fixed - if I'd pointed them at the current branch, the contradiction I just walked you through would quietly stop making sense.

The [architecture document](https://github.com/aaronpowell/tessel-2-revive/blob/bf5ae1081b3c08818d9cd9bf1372c5f84af5249e/docs/architecture.md) I've been quoting from throughout this post didn't exist before I started this work. It got written by reading all seven repos and probing a running board, and then I went through it and checked it. Some of it was wrong and got corrected - you'll see a decent example of that later in the series where a confident conclusion turned out to be backwards. That single coherent picture is what guided everything that comes after it, and it, along with everything else I'll be referring to across this series, lives in [tessel-2-revive](https://github.com/aaronpowell/tessel-2-revive).

My mental model going in was "it's a little microcontroller that runs JavaScript _somehow_." What came out the other side was a deeper insight into the true power of the Tessel 2 - it's a small Linux computer with a microcontroller bolted to the side of it, and that will open up a world more of possibilities than I imagined before getting started. But it also means that the problems are likely to be a lot more complex - it's not just hacking some firmware to run a more modern JavaScript engine, we're dealing with a whole OS here.

## Up next

Now that I know what's on the boards, the next question is whether I can talk to them at all. This will be the first true hurdle to overcome, if I can make them usable from my Windows machine, using the current OpenWrt and Node.js runtime they have, then we can start to think about how to get them upgraded. If I can't even talk to them, then the rest of the work is moot.
